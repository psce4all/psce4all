...
extratag(Cstring t)
{
    ...
    for (i = 0; i < NR(specials); i++) {
        r = &specials[i];
        if ((S(t) > r->szpat) && (strnicmp(T(t), r->pat, r->szpat) == 0))
            return r;
    }
    return 0;
}
...
maybe_tag_or_link(MMIOT *f)
{
    ...
    for (i = 0; i < SZAUTOPREFIX; i++)
        if (strnicmp(text, autoprefix[i], strlen(autoprefix[i])) == 0) {
            Qstring("<a href=\"", f);
            puturl(text, size, f);
            Qstring("\">", f);
            puturl(text, size, f);
    ...
    if (maybeaddress) {
        ...
        Qstring("<a href=\"", f);
        if ((size > 7) && strnicmp(text, "mailto:", 7) == 0)
            mailto = 7;
        else {
            mailto = 0;
